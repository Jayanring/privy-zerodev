...
export type PermissionWithABI<TAbi extends Abi | readonly unknown[], TFunctionName extends string | undefined = string, _FunctionName = TAbi extends Abi ? InferFunctionName<TAbi, TFunctionName> : never> = PermissionCore & {
    abi: TAbi extends Abi ? Narrow<TAbi> : [Narrow<TAbi[number]>];
    functionName: _FunctionName;
    selector?: Hex;
    rules?: never;
} & (TFunctionName extends string ? GetFunctionArgs<TAbi, TFunctionName> : _FunctionName extends string ? GetFunctionArgs<TAbi, _FunctionName> : never);
export type PermissionManual = PermissionCore & {
    abi?: never;
    functionName?: never;
    args?: never;
};
export type Permission<TAbi extends Abi | readonly unknown[], TFunctionName extends string | undefined = string> = PermissionWithABI<TAbi, TFunctionName> | PermissionManual;
